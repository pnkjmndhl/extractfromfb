...
text = re.sub("~[0-9]*~", "", text)  # substitute ~##~ with ""
text = re.sub(r"\\/", " ", text)  # substitute / with ""
text = re.sub(r"[^a-zA-Z ']", "", text)  # anything but A-z and space are removed
text = re.sub(r"WB", " ", text)  # WB and LU are removed
text = re.sub(r"LU", " ", text)
text = re.sub(r"\s+", " ", text)  # multiple spaces converted to single space

# Create stopword list:
stopwords = set(STOPWORDS)
stopwords.update(
    [names_list[0].split(" ")[0], names_list[1].split(" ")[0], "sent", "photo", "ko", "ni", "maa", "video", "haru",
     "bhane", "ra", "Kei", "le", "nai", "ta", "cha", "chai", "na", "ki", "thiye"])

# Generate a word cloud image
wordcloud = WordCloud(stopwords=stopwords, width=2880, height=1620, max_font_size=800, max_words=400,
...
plt.close()
WB = plt.scatter(df_games[df_games.scores.str.contains("WB")]['time'].values,
                 df_games[df_games.scores.str.contains("WB")]['diff'].values, c='red', marker='x')
LU = plt.scatter(df_games[df_games.scores.str.contains("LU")]['time'].values,
                 df_games[df_games.scores.str.contains("LU")]['diff'].values, c='blue', marker='o')
plt.tick_params(labelsize=8)
...
plt.savefig("img/" + name_string + "WB_LU.png", format="png")
